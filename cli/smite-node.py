...
def cmd_update(args):
    ...
    # Update CLI script if it exists in /opt/smite or /opt/smite-node
    cli_source = None
    for source_path in [Path("/opt/smite/cli/smite-node.py"), Path("/opt/smite-node/cli/smite-node.py")]:
        if source_path.exists():
            cli_source = source_path
            break
    
    cli_dest = Path("/usr/local/bin/smite-node")
    if cli_source and cli_dest.exists():
        try:
            import shutil
